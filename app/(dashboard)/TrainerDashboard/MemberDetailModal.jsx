...
function TabBtn({ label, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-black uppercase tracking-tight rounded-lg transition-all cursor-pointer whitespace-nowrap ${
        active
            ...
            <div className="space-y-3">
              ...
              {(day.exercises || []).map((ex, exIdx) => (
                <div key={exIdx} className="space-y-2">
                  <div className="grid grid-cols-[1fr_1fr] sm:grid-cols-[1fr_72px_72px_1fr] gap-2">
                    <input
                      value={ex.name}
                      onChange={(e) => updateExercise(exIdx, "name", e.target.value)}
                      placeholder="Exercise name"
                      className="col-span-2 sm:col-span-1 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-1.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-violet-500"
                    />
                    <input
                      value={ex.sets}
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      className="text-xs px-3 py-1 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 transition cursor-pointer"
                    >
                      Remove
                    </button>
                  </div>
        ...
        {(dietPlan.meals || []).map((meal, idx) => (
          <div key={idx} className="flex gap-2 items-center">
            <input
              ...
              placeholder="7:00 AM"
              className="w-24 shrink-0 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-1.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-violet-500"
            />
            <input
              value={meal.description}
              onChange={(e) => updateMeal(idx, "description", e.target.value)}
              placeholder="e.g. Oats with banana, 2 boiled eggs"
              className="flex-1 min-w-0 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-1.5 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-violet-500"
            />
            <button
              onClick={() => removeMeal(idx)}
              className="w-8 h-8 shrink-0 flex items-center justify-center rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 transition cursor-pointer text-xs"
            >
              ✕
            </button>
          </div>
...
export default function MemberDetailModal({ member, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
      ...
      {/* Panel */}
      <div className="relative z-10 w-full sm:max-w-2xl max-h-[92vh] sm:max-h-[90vh] bg-zinc-950 border border-zinc-800 sm:rounded-2xl rounded-t-2xl flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 border-b border-zinc-800 shrink-0">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center font-black text-sm text-white shadow-md shadow-red-500/20 shrink-0">
              {initials}
            </div>
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 transition cursor-pointer shrink-0 ml-2"
          >
            ✕
          </button>
        </div>
        ...
        {/* Tabs */}
        <div className="flex items-center gap-1 px-3 sm:px-5 py-2 sm:py-3 border-b border-zinc-800 shrink-0 overflow-x-auto no-scrollbar">
          <TabBtn label=" Weekly" active={tab === "weekly"} onClick={() => setTab("weekly")} />
          <TabBtn label=" Diet" active={tab === "diet"} onClick={() => setTab("diet")} />
        ...
        {/* Toast */}
        {toast && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl shadow-lg whitespace-nowrap">
            {toast}
          </div>
        )}
      </div>
