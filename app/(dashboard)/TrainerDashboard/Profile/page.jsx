  ...
  return (
    <div>
      ...
      <select name={name} value={value} onChange={onChange}
        className="w-full bg-zinc-950/50 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-red-500 transition cursor-pointer">
...
export default function TrainerProfilePage() {
  ...
  const cancelEdit = () => {
    setEditSection(null); setPassErrors({});
    if (profile) {
  ...
  const saveInfo = async () => {
    ...
    setSaving(true);
    const { data, ok } = await apiCall("/api/auth/profile", { method: "PUT", body: JSON.stringify({ name: infoForm.name, email: infoForm.email, phone: infoForm.phone }) });
    setSaving(false);
    if (ok && data.success) { setProfile((p) => ({ ...p, ...data.user })); setEditSection(null); showToast("Personal info updated."); }
    else showToast(data.message || "Failed to update.", "error");
  };

  const saveTrainerSettings = async () => {
    setSaving(true);
    const { data, ok } = await apiCall("/api/auth/profile", { method: "PUT", body: JSON.stringify({ specialty: trainerForm.specialty, timings: trainerForm.timings }) });
    setSaving(false);
    if (ok && data.success) { setProfile((p) => ({ ...p, specialty: trainerForm.specialty, timings: trainerForm.timings })); setEditSection(null); showToast("Trainer settings updated."); }
    else showToast(data.message || "Failed to update.", "error");
  };
  ...
  const savePassword = async () => {
    ...
    setPassErrors({});
    setSaving(true);
    const { data, ok } = await apiCall("/api/auth/profile", { method: "PUT", body: JSON.stringify({ currentPassword: passForm.currentPassword, newPassword: passForm.newPassword }) });
    setSaving(false);
    if (ok && data.success) { setPassForm({ currentPassword: "", newPassword: "", confirmPassword: "" }); setEditSection(null); showToast("Password changed successfully."); }
    else showToast(data.message || "Incorrect current password.", "error");
  };
  ...
  const joinedDate = profile?.createdAt ? new Date(profile.createdAt).toLocaleDateString("en-PK", { day: "numeric", month: "long", year: "numeric" }) : "—";
          ...
          <div className="rounded-2xl bg-zinc-900/60 border border-zinc-800/80 p-6 flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <span className="text-xs bg-red-500/10 text-red-400 border border-red-500/20 px-2.5 py-0.5 rounded-full font-bold">Trainer</span>
                {profile?.specialty && (
                  <span className="text-xs text-zinc-400 bg-zinc-800 px-2.5 py-0.5 rounded-full border border-zinc-700">{profile.specialty}</span>
                )}
            ...
            <div className="px-5 py-4 border-b border-zinc-800/80 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-orange-500/10 border border-orange-500/20 flex items-center justify-center">
                  <svg className="w-4 h-4 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                </div>
                <div>
                  <h3 className="font-black text-white uppercase tracking-tight text-sm">Trainer Settings</h3>
                  <p className="text-xs text-zinc-500">Specialty and available timings</p>
                </div>
              </div>
              {editSection !== "trainer" && (
                <button onClick={() => setEditSection("trainer")} className="text-xs px-3 py-1.5 rounded-lg bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white transition cursor-pointer font-semibold">Edit</button>
              )}
            </div>
            <div className="px-5 py-5">
              {editSection !== "trainer" ? (
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <FormSelect label="Specialty" name="specialty" value={trainerForm.specialty} onChange={handleTrainerChange} options={SPECIALTIES} />
                    <FormSelect label="Available Timings" name="timings" value={trainerForm.timings} onChange={handleTrainerChange} options={TIMINGS} />
                  </div>
