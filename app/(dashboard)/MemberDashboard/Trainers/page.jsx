...
export default function MemberTrainersPage() {
  ...
  const handleAssignTrainer = async (trainerId, trainerName) => {
    if (!data?.canChooseTrainer) {
      toast.warning("Your current plan does not allow trainer selection.");
      return;
    }

    const currentTrainer = data?.member?.trainer;

    // If already has a trainer, confirm before proceeding
    if (currentTrainer?.id) {
      const confirmed = await confirm({
        variant: "delete",
        title: "Change Trainer?",
        message: `You currently have "${currentTrainer.name}" as your trainer. Switching to "${trainerName}" will permanently delete your current workout plan and diet plan. This cannot be undone.`,
    ...
    if (ok && result.success) {
      ...
      toast.success(`${trainerName} is now your trainer.`);
    } else {
      toast.error(result.message || "Failed to assign trainer.");
    }
  ...
  const filteredTrainers = trainers.filter((t) =>
    [t.name, t.specialty, t.timings, t.phone].some((v) => v?.toLowerCase().includes(search.toLowerCase()))
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-zinc-950 to-zinc-900 text-neutral-100 font-sans selection:bg-red-500 selection:text-white">
      ...
      <div className="lg:ml-60 flex flex-col min-h-screen pt-14 lg:pt-0">
        ...
        <main className="flex-1 px-4 sm:px-8 py-6 space-y-6 max-w-6xl mx-auto w-full">

          {/* Hero Banner */}
          <section className="relative rounded-2xl overflow-hidden border border-zinc-800/80 p-5 md:p-8">
            ...
            <div className="relative grid gap-5 md:grid-cols-[1.4fr_0.8fr] items-start">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-red-500 mb-2">{quote}</p>
                <h2 className="text-2xl md:text-4xl font-black uppercase tracking-tight text-white">Pick a trainer that matches your goal.</h2>
                <div className="flex items-center gap-2 mt-3">
                  <div className="h-0.5 w-12 bg-gradient-to-r from-red-500 to-orange-500 rounded-full" />
                  <div className="h-0.5 w-4 opacity-60 bg-gradient-to-r from-red-500 to-orange-500 rounded-full" />
                  <div className="h-0.5 w-2 opacity-30 bg-gradient-to-r from-red-500 to-orange-500 rounded-full" />
                </div>
              ...
              <div className="rounded-2xl border border-zinc-800/80 bg-zinc-950/70 p-4 sm:p-5">
                ...
                <p className="mt-2 text-base font-black text-white uppercase tracking-tight">{member.plan ? member.plan.name : "No active plan"}</p>
                <p className="mt-1 text-sm text-zinc-400">{member.plan ? `${member.plan.duration} · Rs ${member.plan.price?.toLocaleString?.() || member.plan.price}` : "Subscribe to unlock trainer selection."}</p>
                <div className={`mt-3 inline-flex text-xs px-3 py-1 rounded-full border font-semibold ${canChooseTrainer ? "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" : "text-zinc-400 bg-zinc-800 border-zinc-700"}`}>
          ...
          {member.trainer && (
            <div className="flex items-start gap-3 p-4 rounded-2xl border border-orange-500/20 bg-orange-500/5">
              <svg className="w-4 h-4 text-orange-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
              </svg>
              <div>
                <p className="text-xs font-black uppercase tracking-wider text-orange-400">Current Trainer</p>
                <p className="font-semibold text-white mt-0.5">{member.trainer.name}</p>
                <p className="text-xs text-zinc-400 mt-0.5">{member.trainer.specialty} · {member.trainer.timings}</p>
                <p className="text-xs text-amber-400/80 mt-1">Choosing a new trainer will permanently delete your current workout and diet plan.</p>
              </div>
            </div>
          )}

          {/* Trainer list */}
          <section className="rounded-2xl bg-zinc-900/60 border border-zinc-800/80 p-4 sm:p-5 space-y-4">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              ...
              <div className="relative w-full md:w-80">
                ...
                <input type="text" placeholder="Search trainers..." value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  className="w-full pl-9 pr-4 py-2.5 bg-zinc-950/50 border border-zinc-800 rounded-xl text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-red-500 transition" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
              {filteredTrainers.length === 0 ? (
                ...
                const trainerInitials = trainer.name.split(" ").map((p) => p[0]).join("").toUpperCase().slice(0, 2);
                const isCurrent = trainer.id === activeTrainerId;
                return (
                  <div key={trainer.id} className="rounded-2xl border border-zinc-800 bg-zinc-950/40 p-4 sm:p-5 flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3 min-w-0">
                      <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center font-bold text-sm shrink-0 text-white shadow-lg shadow-red-500/25">
                        {trainerInitials}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-white truncate">{trainer.name}</p>
                        <p className="text-xs text-zinc-400 mt-0.5">{trainer.specialty}</p>
                    ...
                    <button
                      ...
                      onClick={() => handleAssignTrainer(trainer.id, trainer.name)}
                      className={`shrink-0 px-3 sm:px-4 py-2 rounded-xl text-xs font-black border transition cursor-pointer disabled:cursor-not-allowed disabled:opacity-50 ${
                        isCurrent
