...
function EditMemberModal({ member, plans, trainers, onClose, onSaved }) {
  const [planId, setPlanId]       = useState(member.plan?._id || "");
  const [trainerId, setTrainerId] = useState(member.assignedTrainer?._id || "");
  ...
  const handleSave = async () => {
    ...
    if (planId    !== (member.plan?._id || ""))             body.planId = planId || null;
    if (trainerId !== (member.assignedTrainer?._id || ""))  body.assignedTrainerId = trainerId || null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4">
      <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-6 space-y-5 text-white">
        {/* header */}
        <div className="flex items-center justify-between pb-3 border-b border-zinc-800">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center font-black text-sm shadow-md">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select value={trainerId} onChange={(e) => setTrainerId(e.target.value)} className={sel}>
    ...
    <div className="min-h-screen bg-zinc-950 text-white font-sans">
      ...
      <div className="lg:ml-60 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-20 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800/60 px-6 py-4 flex items-center justify-between">
          ...
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center font-bold text-sm shadow-lg shadow-red-500/20">
            GM
          </div>
        </header>
        ...
        <main className="flex-1 px-6 py-6 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Total Members", value: counts.total,    accent: "from-red-500 to-orange-500",    big: "ALL"  },
              { label: "Active",        value: counts.active,   accent: "from-emerald-500 to-teal-500",  big: "ACT"  },
              { label: "Expired",       value: counts.expired,  accent: "from-amber-500 to-yellow-500",  big: "EXP"  },
              { label: "No Plan",       value: counts.inactive, accent: "from-zinc-500 to-zinc-600",     big: "OFF"  },
              ...
              <div key={label} className="relative rounded-2xl overflow-hidden bg-zinc-900/80 border border-zinc-800 hover:border-zinc-700 p-5 group transition">
                <span className="absolute -bottom-2 -right-1 text-5xl font-black uppercase text-white/[0.04] select-none pointer-events-none leading-none">{big}</span>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-zinc-800 text-zinc-500 text-xs uppercase tracking-wider">
                      ...
                      <th className="text-left px-5 py-3 hidden lg:table-cell">Trainer</th>
                      ...
                      <tr key={m._id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-red-500/20 to-orange-500/20 flex items-center justify-center text-xs font-black text-red-400 shrink-0">
                              {initials(m.name)}
                            </div>
                            <span className="font-semibold text-white">{m.name}</span>
                          </div>
                        </td>
                        <td className="px-5 py-3 text-zinc-400 hidden sm:table-cell">{m.email}</td>
                        <td className="px-5 py-3 text-zinc-400 hidden md:table-cell">
                          {m.plan ? (
                            <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full border bg-zinc-800 border-zinc-700 text-zinc-300">
                              {m.subscriptionBadge?.icon} {m.plan.name}
                            </span>
                          ) : (
